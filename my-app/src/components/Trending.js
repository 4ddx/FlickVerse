...
const Trending = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=2ba6edcaaa42bee1b424fac71c312fa9&language=en-US&page=1`);
  ...
  return (
    <div className='flex flex-col w-full p-5 pb-0 gap-2'>
      <div className='font-bold text-3xl ml-1 pb-3 hover:text-red-300 duration-300'>Trending</div>
      ...
      {selectedMovie && (
        <div className='fixed top-0 left-0 w-screen h-screen bg-opacity-80 bg-black flex justify-center items-center'>
          <div className='bg-black p-4'>
            ...
            <h2 className='text-xl font-bold'>{selectedMovie.original_title}</h2>
